...
class Settings(BaseSettings):
    ...
    # ==================== 数据库连接地址 ====================
    mysql_dsn: str = "mysql+pymysql://root:*@localhost:3306/mysqla"  # MySQL 数据库连接字符串（用户名:密码@地址:端口/库名）
    ...
    # ==================== 用户登录鉴权（JWT） ====================
    jwt_secret: str = "*"  # JWT 签名密钥，用于加密和验证用户登录 Token，生产环境必须改成随机字符串
    ...
    def validate_runtime(self) -> None:
        ...
        if self.neo4j_enabled and self.neo4j_password in {"", "*"}:
