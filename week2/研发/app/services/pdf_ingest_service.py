...
class PDFIngestService:
    ...
    def _describe_image(image_bytes: bytes, ext: str = "png") -> str:
        """调用配置中的视觉模型"Qwen/Qwen3-VL-8B-Instruct"，为 PDF 中提取出的图片生成中文描述。
        图片会被编码为 base64 data URL，并通过 OpenAI 兼容的 chat/completions 接口发送。
        如果外部接口调用失败，则返回空字符串，不中断 PDF 入库流程。
        PyMuPDF 逐页处理流程（每页独立）：
          ├─ 第1步：表格提取 → _extract_tables_as_markdown()
          │            用 page.find_tables() 检测表格边界
          │            第一行作表头，其余作表体
          │            输出 Markdown 格式表格
          │
          ├─ 第2步：文本层提取 → page.get_text("text")
          │            普通 PDF 直接提取文字层
          │
          ├─ 第3步：OCR 兜底（文本 < 30 字符时触发）→ _ocr_page()
          │            页面 2 倍缩放渲染为图片
          │            RapidOCR（本地 ONNX）识别文字
          │            OCR 结果替换原文本层
          │
          └─ 第4步：图片描述 → _extract_images_as_text()
                       提取页面内嵌图片（>100×100 像素）
                       前 3 张图片送入视觉大模型
                       生成中文描述追加到页面文本
        """
...
Neo4j 不参与 0.6/0.4 的加权公式，而是作为额外候选加入。如果一个 chunk 同时被 BM25/向量检索到，又被 Neo4j 图谱关联，它的可信度提高。
Neo4j 负责把"谁和谁是股东关系""谁控制了谁"这类结构化关系转成文本片段，既参与检索排序竞争，又直接作为补充上下文交给大模型，解决纯文本 RAG 无法回答股权/关联方/控制关系的问题。
