...
def Talker_response_img(question_audio, method, text, voice, rate, volume, pitch,
    ...
    if method == 'SadTalker':
        try:
            ...
            gr.Error(f"SadTalker生成失败：{error}")
            return None
...
def human_response(source_image, history, question_audio, talker_method, voice, rate, volume, pitch,
    ...
    if not isinstance(last_turn, (list, tuple)) or len(last_turn) < 2:
        gr.Warning("多轮对话历史格式异常，请清空后重试")
        return None

    question = last_turn[0]
    response = last_turn[1]
    if not question or not response:
        gr.Warning("请先完成一轮有效对话")
        return None
...
def MuseTalker_response(source_video, bbox_shift, question_audio, text, voice,
    ...
    if 'musetalker' not in globals() or musetalker is None:
        gr.Warning("MuseTalk环境未就绪，请先安装模型和依赖")
        return None
...
def musetalk_prepare_material(source_video, bbox_shift):
    ...
    if 'musetalker' not in globals() or musetalker is None:
        gr.Warning("MuseTalk环境未就绪，请先安装模型和依赖")
        return source_video, "MuseTalk环境未就绪"
