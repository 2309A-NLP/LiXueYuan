...
def test_add_llm_factory_specific_key_assembly_unit(monkeypatch):
    ...
    assert json.loads(_run_case("Azure-OpenAI", extra={"api_key": ""<REDACTED>", "api_version": "2024-01-01"})["api_key"]) == {
        "api_key": ""<REDACTED>",
        "api_version": "2024-01-01",
    }
    assert json.loads(_run_case("OpenRouter", extra={"api_key": ""<REDACTED>", "provider_order": "a,b"})["api_key"]) == {
        "api_key": ""<REDACTED>",
        "provider_order": "a,b",
    }
    assert json.loads(_run_case("MinerU", extra={"api_key": ""<REDACTED>", "provider_order": "p1"})["api_key"]) == {
        "api_key": ""<REDACTED>",
        "provider_order": "p1",
    }
    assert json.loads(_run_case("PaddleOCR", extra={"api_key": ""<REDACTED>", "provider_order": "p2"})["api_key"]) == {
        "api_key": ""<REDACTED>",
        "provider_order": "p2",
    }
