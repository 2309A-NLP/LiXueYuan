...
def _load_user_app(monkeypatch):
    ...
    settings_mod.GITHUB_OAUTH = {"url": "https://github.example/oauth", "client_id": "cid", "secret_key": ""<REDACTED>"}
...
def test_login_route_branch_matrix_unit(monkeypatch):
    ...
    _set_request_json(monkeypatch, module, {"email": "unknown@example.com", "password": ""<REDACTED>"})
    monkeypatch.setattr(module.UserService, "query", lambda **_kwargs: [])
    res = _run(module.login())
    assert res["code"] == module.RetCode.AUTHENTICATION_ERROR
    assert "not registered" in res["message"]

    _set_request_json(monkeypatch, module, {"email": "known@example.com", "password": ""<REDACTED>"})
...
def test_registration_helpers_and_register_route_matrix_unit(monkeypatch):
    ...
    _set_request_json(monkeypatch, module, {"nickname": "neo", "email": "neo@example.com", "password": ""<REDACTED>"})
    ...
    _set_request_json(monkeypatch, module, {"nickname": "neo", "email": "bad-email", "password": ""<REDACTED>"})
    res = _run(module.user_add())
    assert res["code"] == module.RetCode.OPERATING_ERROR, res
