...
def test_list_app_grouping_availability_and_merge(monkeypatch):
    ...
    tenant_rows = [
        _TenantLLMRow(id=1, llm_name="fast-emb", llm_factory="FastEmbed", model_type="embedding", api_key=""<REDACTED>", status="1"),
        _TenantLLMRow(id=2, llm_name="tenant-only", llm_factory="CustomFactory", model_type="chat", api_key=""<REDACTED>", status="1"),
        _TenantLLMRow(id=3, llm_name="gpt-5.5", llm_factory="OpenAI", model_type="chat", api_key=""<REDACTED>", status="1"),
        _TenantLLMRow(id=4, llm_name="gpt-5.4", llm_factory="OpenAI", model_type="chat", api_key=""<REDACTED>", status="1"),
    ]
...
def test_set_api_key_model_probe_matrix_unit(monkeypatch):
    ...
    req = {"llm_factory": factory, "api_key": ""<REDACTED>", "base_url": "http://x", "verify": True}
...
def test_add_llm_factory_specific_key_assembly_unit(monkeypatch):
    ...
    def _run_case(factory, *, model_type=module.LLMType.CHAT.value, extra=None):
        req = {"llm_factory": factory, "llm_name": "model", "model_type": model_type, "api_key": ""<REDACTED>", "api_base": "http://api"}
        if extra:
            req.update(extra)
        _set_request_json(monkeypatch, module, req)
    ...
    volc = _run_case("VolcEngine", extra={"ark_api_key": ""<REDACTED>", "endpoint_id": "eid"})
    assert json.loads(volc["api_key"]) == {"ark_api_key": ""<REDACTED>", "endpoint_id": "eid"}
    ...
    spark_chat = _run_case("XunFei Spark", extra={"spark_api_password": ""<REDACTED>"})
    ...
    azure = _run_case("Azure-OpenAI", extra={"api_key": ""<REDACTED>", "api_version": "2024-01-01"})
    assert json.loads(azure["api_key"]) == {"api_key": ""<REDACTED>", "api_version": "2024-01-01"}
    openrouter = _run_case("OpenRouter", extra={"api_key": ""<REDACTED>", "provider_order": "a,b"})
    assert json.loads(openrouter["api_key"]) == {"api_key": ""<REDACTED>", "provider_order": "a,b"}
    mineru = _run_case("MinerU", extra={"api_key": ""<REDACTED>", "provider_order": "p1"})
    assert json.loads(mineru["api_key"]) == {"api_key": ""<REDACTED>", "provider_order": "p1"}
    paddle = _run_case("PaddleOCR", extra={"api_key": ""<REDACTED>", "provider_order": "p2"})
    assert json.loads(paddle["api_key"]) == {"api_key": ""<REDACTED>", "provider_order": "p2"}
...
def test_add_llm_model_type_probe_and_persistence_matrix_unit(monkeypatch):
    ...
    res = _call({"llm_factory": "FChatPass", "llm_name": "m", "model_type": module.LLMType.CHAT.value, "api_key": ""<REDACTED>"})
    assert res["code"] == 0, res["message"]
    assert res["data"] is True
    assert saved
