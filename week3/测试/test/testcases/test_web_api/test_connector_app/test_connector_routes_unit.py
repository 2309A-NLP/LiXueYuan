...
def test_start_google_web_oauth_matrix(monkeypatch):
    ...
    monkeypatch.setattr(module, "get_request_json", lambda: _AwaitableValue({"credentials": ""<REDACTED>"}))
...
def test_box_oauth_start_callback_and_poll_matrix(monkeypatch):
    ...
    monkeypatch.setattr(
        module,
        "get_request_json",
        lambda: _AwaitableValue({"client_id": "cid", "client_secret": ""<REDACTED>", "redirect_uri": "https://box.local/callback"}),
    )
    ...
    redis.store[module._web_state_cache_key("flow-box", "box")] = json.dumps(
        {"user_id": "tenant-1", "client_id": "cid", "client_secret": ""<REDACTED>"}
    )
